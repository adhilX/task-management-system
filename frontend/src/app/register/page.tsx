...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    setErrorMsg(null);
    try {
      // First registration is Admin, subsequent registration is restricted
      await signup(data);
      setSuccess(true);
      setTimeout(() => {
        router.push('/login');
    ...
    <div className="flex min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="relative hidden w-1/2 flex-col justify-between bg-gradient-to-br from-indigo-950 via-slate-900 to-violet-950 p-12 lg:flex border-r border-slate-800">
        ...
        <div className="my-auto max-w-lg z-10 space-y-6">
          ...
          <p className="text-lg text-slate-400">
            Create the primary administrator account to bootstrap your company's JiraFlow workspace.
          </p>
      ...
      <div className="flex w-full flex-col justify-center px-6 py-12 lg:w-1/2 lg:px-24 bg-slate-950">
        <div className="mx-auto w-full max-w-md space-y-8">
          ...
          {success && (
            <div className="rounded-lg bg-emerald-950/50 border border-emerald-500/50 p-4 text-sm text-emerald-400">
              <strong>Workspace initialized successfully!</strong> Redirecting to sign in screen...
            </div>
          )}

          {isLocked && (
            <div className="flex flex-col space-y-4 rounded-lg bg-amber-950/40 border border-amber-500/40 p-5 text-sm text-amber-300">
